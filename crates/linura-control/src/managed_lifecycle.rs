...
use linura_core::{
    Actor, ActorKind, ApprovalRequestId, PrincipalId, ProviderId, ResourceId,
};
...
/// Human identity asserted only by a trusted privileged local boundary.
///
/// This type is not exposed through `linura-sdk` or any transport wire. Constructing
/// it is an explicit assertion by the trusted composition root that the local
/// administrator has already been authenticated outside `linura-control`.
...
/// One-shot handoff object produced only after durable authority crosses the
/// `Prepared -> Indeterminate` boundary. It cannot be cloned and the executor
/// port must consume it by value before it can obtain serializable correlation
/// material.
#[derive(Debug)]
...
impl AuthorizedEffect {
    ...
    /// Consume the process-local permit and release only the exact effect and
    /// derived executor correlation binding to the trusted executor adapter.
    #[must_use]
...
/// Trusted adapter from Control's one-shot authority handoff to a concrete
/// privileged executor. Implementations must not persist or replay the request.
...
/// Structurally independent postcondition verifier.
///
/// Implementations must obtain their own fresh authoritative observation and
/// must not accept executor receipts as machine-state proof.
...
/// v0.6 Control-owned orchestrator for the first complete, narrowly supported
/// managed external effect.
///
/// The only admitted effect is `active_state={active|inactive}` for a canonical
/// systemd `.service` unit in the `linura-managed-` namespace. The planner,
/// trusted risk/policy review, administrator approval, durable transaction,
/// one-shot handoff, concrete executor, independent verifier, commit audit and
/// reconciliation all remain distinct boundaries.
#[derive(Debug)]
    ...
    S: TransactionStore,
{
        ...
        V: IndependentManagedVerifier,
    {
        ...
        let candidate = match self.authority.candidate(
            ...
            Err(DurableAuthorityError::CandidateNotMutation) => None,
            Err(error) => return Err(error.into()),
        };

        if let Some(candidate) = candidate {
            let effect = effect_from_candidate(&candidate)?;
            let approval_evidence_id = self.authorize_candidate(&candidate, approval)?;
            match self.authority.prepare(candidate, approval_evidence_id) {
                Ok(mut prepared) => {
                    ...
                    if execution.dispatch_digest.to_hex() != dispatch_digest {
                        return Err(ManagedLifecycleError::Contract(
                            "executor returned a dispatch digest different from the authorized handoff"
                                .into(),
                        ));
                    }
                    ...
                    if verification.disposition != VerificationDisposition::Satisfied {
                        return Err(match verification.disposition {
                            VerificationDisposition::NotSatisfied => {
                                ManagedLifecycleError::VerificationNotSatisfied(verification.detail)
                            }
                            VerificationDisposition::Inconclusive => {
                                ManagedLifecycleError::Indeterminate(verification.detail)
                            }
                            VerificationDisposition::Satisfied => unreachable!(),
                        });
                    }
                ...
                Err(DurableAuthorityError::Transaction(detail))
                    if detail == "durable request idempotency conflict" =>
                {
            ...
            match self.authority.snapshot(&transaction_id) {
                ...
                {
                    Err(ManagedLifecycleError::UnsupportedEffect(
                        "requested state already holds and no managed transaction exists".into(),
                    ))
                }
                Err(error) => Err(error.into()),
            }
        ...
        V: IndependentManagedVerifier,
    {
        ...
        match snapshot.state {
            TransactionState::Prepared => {
                // The exact prepared binding still exists in this process only when
                // an earlier invocation failed before handoff. Rebuild the exact
                // candidate and require an explicit fresh handoff.
                ...
                if execution.disposition == ExecutionDisposition::RejectedBeforeDispatch {
                    return Err(ManagedLifecycleError::ExecutionRejected(execution.detail));
                }
                self.finish_recovered(
                    principal,
                    actor,
                    request,
                    effect,
            ...
            TransactionState::Indeterminate => self.finish_recovered(
                principal,
                actor,
                request,
                effect,
            ...
            TransactionState::Committed => {
                ...
                if verification.disposition != VerificationDisposition::Satisfied {
                    return Err(ManagedLifecycleError::Reconciliation(format!(
                        "committed state no longer satisfies its managed postcondition: {}",
                        verification.detail
                    )));
                }
                let mut progress = progress_through(MutationStage::Commit)?;
            ...
            TransactionState::Aborted | TransactionState::RecoveryBlocked => {
                Err(ManagedLifecycleError::TerminalState(format!(
                    "{} is {}; use a new request id after reviewing durable evidence",
    ...
    #[allow(clippy::too_many_arguments)]
    fn finish_recovered<V>(
        &mut self,
        principal: AuthenticatedPrincipal,
        actor: Actor,
        request: PlanDesiredStateRequest,
        approval_effect: EffectDescriptor,
        transaction_id: TransactionId,
        plan_id: String,
        ...
        V: IndependentManagedVerifier,
    {
        ...
        let approval_evidence_id = if verification.disposition == VerificationDisposition::NotSatisfied
        {
            ...
            let trusted = TrustedHumanApproval {
                principal: PrincipalId::new(principal.as_str().to_owned())
                    .map_err(|error| ManagedLifecycleError::Contract(error.to_string()))?,
            };
            self.authorize_candidate(&candidate, &trusted)?
        } else {
            None
        };

        let recovery = if verification.disposition == VerificationDisposition::Inconclusive {
        ...
        match recovery {
            DurableRecoveryOutcome::Verified(verified) => {
                ...
                self.authority.integrity_check()?;
                advance(&mut progress, MutationStage::Audit)?;
                self.reconcile(&principal, &actor, &request, &approval_effect, verifier)?;
                advance(&mut progress, MutationStage::Reconcile)?;
                Ok(receipt(
                    &transaction_id,
                    &plan_id,
...
fn effect_from_candidate(
    ...
    let desired = std::collections::BTreeMap::from([(change.key.clone(), change.desired.clone())]);
    effect_from_parts(
...
fn effect_from_parts(
    provider: ProviderId,
    resource: ResourceId,
    desired_state: &std::collections::BTreeMap<String, String>,
) -> Result<EffectDescriptor, ManagedLifecycleError> {
...
mod tests {
    ...
    use linura_core::{CapabilityId, RequestId, SemanticReason};

    use super::*;

    fn request(resource: &str, desired: &str) -> PlanDesiredStateRequest {
        PlanDesiredStateRequest {
            request_id: RequestId::new("request:v06:test")
