...
impl Observer for SystemdObserver {
    ...
    fn observe_authoritative(
        ...
        .map_err(provider_bus_error)?;
        let id: String = unit.get_property("Id").map_err(provider_bus_error)?;
        let description: String = unit
            .get_property("Description")
            .map_err(provider_bus_error)?;
        let load_state: String = unit.get_property("LoadState").map_err(provider_bus_error)?;
        let active_state: String = unit
            .get_property("ActiveState")
            .map_err(provider_bus_error)?;
        let sub_state: String = unit.get_property("SubState").map_err(provider_bus_error)?;
        let fragment_path: String = unit
...
impl NetworkManagerObserver {
    ...
    fn observe_manager(
        ...
        let version: String = manager
            ...
            .map_err(provider_bus_error)?;
        let state: u32 = manager.get_property("State").map_err(provider_bus_error)?;
        let connectivity: u32 = manager
            .get_property("Connectivity")
            .map_err(provider_bus_error)?;
        let networking_enabled: bool = manager
    ...
    fn observe_device(
        ...
        .map_err(provider_bus_error)?;
        let observed_interface: String = device
            .get_property("Interface")
            .map_err(provider_bus_error)?;
        let state: u32 = device.get_property("State").map_err(provider_bus_error)?;
        let device_type: u32 = device
            .get_property("DeviceType")
            .map_err(provider_bus_error)?;
        let managed: bool = device.get_property("Managed").map_err(provider_bus_error)?;
        let driver: String = device.get_property("Driver").map_err(provider_bus_error)?;
        Ok(ObservationEnvelope {
            provider: provider_id(NM_PROVIDER),
            resource: resource.clone(),
            capability: capability.clone(),
