...
- Exact-source release-snapshot gating, immutable-tag dispatch, exact-source candidate build, automatic verified promotion and independent published-byte verification.
- SPDX SBOMs, SHA-256 checksums, machine-readable release evidence and GitHub/Sigstore build provenance for the candidate artifact set.
...
Release publication authority is also bounded: the release-intent commit becomes one immutable release snapshot in protected `main` history; permanent CI, Security and CodeQL gates must succeed for that same exact SHA before version-tag binding; source ancestry is re-proven immediately before tag creation; candidate and promotion stages operate on immutable source/tag evidence and do not rebuild during promotion. `main` may legitimately advance after the release snapshot is established without changing the identity or evidence of that snapshot.
...
- Release-intent dispatch, trusted candidate handoff, automatic verified promotion and explicit independent verification. [PR #9](https://github.com/linura-org/linura/pull/9)
- Release-dispatch correction: valid workflow expression syntax and an explicit immutable exact-source release-snapshot invariant that avoids claiming an unsupported cross-ref atomicity guarantee. [PR #10](https://github.com/linura-org/linura/pull/10)
