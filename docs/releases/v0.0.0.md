...
- Exact-main release-intent gating, immutable-tag dispatch, exact-source candidate build, automatic verified promotion and independent published-byte verification.
...
Release publication authority is also bounded: an exact `main` release-intent commit cannot create a tag until permanent CI, Security and CodeQL gates have succeeded for that same SHA; the dispatcher revalidates the current `main` head immediately before tag mutation; candidate and promotion stages operate on immutable source/tag evidence and do not rebuild during promotion.
...
- Exact-main release-intent dispatch, trusted candidate handoff, automatic verified promotion and explicit independent verification. [PR #9](https://github.com/linura-org/linura/pull/9)
- Release-dispatch correction: valid workflow expression syntax plus fail-closed current-main revalidation immediately before tag mutation. [PR #10](https://github.com/linura-org/linura/pull/10)
