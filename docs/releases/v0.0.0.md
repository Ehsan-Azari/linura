...
- Explicit exact-source release selection, immutable version tagging, exact-source candidate build, automatic verified promotion and independent published-byte verification.
...
Release publication authority is also bounded: the exact release source is selected only after protected review plus successful exact-SHA CI, Security and CodeQL. The immutable `v0.0.0` tag is then created once at that chosen source. No asynchronous dispatcher is permitted to choose between competing same-version snapshots. Candidate and promotion stages operate on immutable source/tag evidence and promotion does not rebuild.
...
- configured review must complete and blocking review threads must be addressed on the release changes;
...
- the immutable `v0.0.0` tag must bind to the explicitly selected reviewed source SHA;
...
- Trusted candidate handoff, automatic verified promotion and explicit independent verification. [PR #9](https://github.com/linura-org/linura/pull/9)
- Release-control correction: removal of asynchronous automatic tag selection in favor of an explicit, review-complete exact-source tag authority. [PR #10](https://github.com/linura-org/linura/pull/10)
...
No Git tag or GitHub Release alone establishes v0.0.0 completion. Publication evidence exists only after the chosen exact source passes the protected review/gate lifecycle, the exact candidate bytes are promoted, and independent verification revalidates checksums, provenance, evidence metadata and release-note identity.
