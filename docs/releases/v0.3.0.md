...
- fresh protected-main CI, Security, and CodeQL for the exact tree-identical `release:` authorization source;
...
Publication evidence exists only after the tree-identical release-intent source passes protected-main gates, Trusted Release Proof succeeds with both mandatory VM qualifications and independent reproduction, promotion revalidates and consumes the sealed proof output, final publication creates the `v0.3.0` tag and immutable GitHub Release from the exact proven bytes, and independent published-release verification succeeds.
